...
:class:`Sweep` runs the openCREST binary once per cell in a parameter
...
3. Waits ``duration_s`` (or until ``stop_condition`` returns true), then
...
The orchestrator is agnostic to what the experiment is doing *inside* a
cell; experiment-specific behaviour (CDC commands, stop conditions,
post-processing) is supplied via callbacks. The openCREST binary itself
is opaque.

Long-sweep hardening:

* ``progress=True`` (default) prints a one-line transition for every cell
  start and completion to ``progress_stream`` (stderr by default).
* ``max_cell_runtime_s`` is an optional hard kill timer, independent of
  ``duration_s`` / ``stop_condition``. Use it as a safety net for
  message-count-driven experiments that set ``duration_s`` to infinity.
...
class Sweep:
    ...
    duration_s
        ...
        cell is expected to exit cleanly. ``stop_condition`` may terminate
        sooner. Set to ``float("inf")`` for message-count-driven cells
        (pair with ``max_cell_runtime_s``).
    parallel
        Number of cells to run concurrently. Default 1 because two-modem
        scenarios contend for the same USB devices.
    extra_params
        Constant parameters that apply to every cell (e.g. modem serials,
        ``channel_gain_db``). Merged into each cell's params before
        rendering.
    pre_run, post_run
    ...
    stop_condition
        ...
        ``True`` makes the runner SIGTERM the cell early (counts as
        success).
    max_cell_runtime_s
        Optional hard kill timer. If set, a cell that exceeds this
        wall-clock budget is forcefully terminated and marked **failed**
        with a watchdog reason in ``CellResult.error``. Distinct from
        ``duration_s``: the soft limit is the planned runtime; the
        watchdog is a safety net for ``duration_s=inf`` cells. ``None``
    ...
    retries
        ...
        the final attempt's logs are what survive.
    progress, progress_stream
        When ``progress`` is true (default), every cell start and
        completion emits a single line to ``progress_stream``
        (``sys.stderr`` by default). Set ``progress=False`` to mute.
    """
    ...
    def cells(self) -> list[Params]:
        """Materialise the parameter cross-product into a list of dicts."""
    ...
    def run(self) -> list[CellResult]:
        """Execute every cell and return the per-cell result list. Calling
        ``run()`` again re-executes every cell from scratch and overwrites
        outputs."""
        ...
        if self._progress_total > 0 and self.duration_s != float("inf") \
            ...
            # Soft upper bound on total wall-clock budget; with infinite
            # duration_s and no watchdog there's no meaningful estimate.
        ...
        # Stable ordering for the index CSV.
    ...
    def collect_results(self):
        ...
        one row per message event. Thin wrapper around
        :func:`metrics_loader.load_sweep`.
        """
    ...
    def _run_cell_inner(self, params: Params, cell_id: CellId,
        ...
        # Override output_directory so artifacts land inside the per-cell
        # folder regardless of the template's default.
        ...
        # Recover the scenario name from the YAML so the summary path is
        # predictable.
        try:
        ...
        # Simulator writes <output_dir>/<name>_summary.json by default.
        ...
        try:
            proc = subprocess.Popen(
                ...
                stdout=stdout_log,
                stderr=stderr_log,
                # New process group so SIGTERM can reach any children the
                # binary spawned.
        ...
        if watchdog_reason is not None:
            # Watchdog takes precedence over the exit-code message — the
            # cell was forcibly killed.
            err = watchdog_reason
    ...
    def _wait_or_terminate(self, handle: CellHandle,
        ...
        """Poll until the process exits naturally, ``duration_s`` elapses,
        ``stop_condition`` returns true, or the watchdog fires.

        Returns ``(exit_code, watchdog_reason)``. ``watchdog_reason`` is
        ``None`` for natural exit / soft-limit / stop-condition paths and a
        short string when ``max_cell_runtime_s`` triggered the kill; in
        the latter case the cell is treated as failed regardless of exit
        code.
        """
        ...
        while True:
            ...
            if self.stop_condition is not None and self.stop_condition(handle):
                break
            # Sleep until the next deadline or poll tick, whichever is
            # sooner; cap at poll_interval_s so stop_condition is exercised
        ...
        # Graceful SIGTERM for soft-limit / stop_condition.
    ...
    def _emit_sweep_start(self, total: int,
        ...
        if budget_per_cell is None:
            ...
            return
        # Upper bound assumes every cell uses its full budget.
    ...
    def _emit_cell_start(self, cell_id: CellId, params: Params) -> None:
        if not self.progress:
            return
        # Only show the sweep-axis parameters (the ones cells differ in).
    ...
    def _emit_cell_retry(self, cell_id: CellId, attempt: int,
        ...
        if not self.progress:
            return
        # Trim long error messages to keep retry lines readable.

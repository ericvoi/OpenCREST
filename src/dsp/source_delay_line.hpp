...
// each per-tap consumer reads at a fractional position evolving block-
// to-block according to that tap's time-varying delay. This is the only
// piece of read-style state in the geometric pipeline.
//
...
// The recompute_geometric_taps clamp in Channel keeps reads inside the
// configured envelope, so read positions never lag the producer by more
// than `capacity − 1` source samples.
//

...
// Per-channel physically-derived gain (dB). Chains the source-side
// voltage anchor + TX monitor pad + TVR, the acoustic path loss at the
// source's centre frequency, and the receiver-side RVR + RX injection
// pad + DAC voltage anchor.
//
//   physical_gain_dB =
//         20·log10(V_ref_adc_src_peak)            // ADC FS in V
//       + (-output_atten_src_dB)                  // un-do TX monitor pad
//       + tvr_src_db                              // V_drive → µPa @ 1 m
//       - TL_dB(range, src_cal.center_freq_hz)    // Thorp at source freq
...
// fc = 25 kHz) and TransducerSpec{} (TVR = RVR = 0 dB) this collapses
// to -TL_dB at the default centre frequency.
//
// Result is dB-amplitude (20·log10 of the linear scalar mapping a source
// ADC sample to the corresponding receiver DAC sample). Pure,
// allocation-free, safe at scenario load.

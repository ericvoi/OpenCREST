...
// bucket_idx = log(us) * kScale; bucket 0 covers [0, kMinUs), bucket
// kBucketCount-1 holds the open-ended >= kMaxUs tail.
...
size_t ProcessingTimeStats::bucket_for(uint64_t duration_us) {
    ...
    // Clamp log→bucket mapping at both ends.
...
void ProcessingTimeStats::record(uint64_t duration_us, uint64_t deadline_us) {
    ...
    // Atomic max via CAS loop; one source thread typically completes on the
    // first try.
...
ProcessingTimeStats::Snapshot ProcessingTimeStats::snapshot() const {
    ...
    // Percentile via cumulative bucket scan; reported value is the bucket
    // lower-bound (one bucket of granularity).
